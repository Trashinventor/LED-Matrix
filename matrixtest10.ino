...
uint8_t clock = 9;
uint8_t Reset = 8;
uint8_t latchPinPORTB = latchPin - 8;
uint8_t clockPinPORTB = clockPin - 8;
uint8_t dataPinPORTB = dataPin - 8;
uint8_t i = 0;
...
unsigned int scrollSpeed = 100;                        // Speed of scrolling in milliseconds
...
void scrollText(const char* , int , unsigned int);
...
void displayAnimation(const byte* frames[], int , unsigned int );
...
void displayCharacter(const byte character[6], int startX) {
  ...
  for (uint8_t row = 0; row < 6; row++) {
    ...
    for (uint8_t bit = 0; bit < 8; bit++) {
      uint8_t x = startX + bit;                                         // Calculate the column index
      if (x < 24) {                                       // Ensure pixel stays within buffer bounds
        uint8_t byteIndex = x / 8;                                      // Determine the byte index in temporaryBuffer
...
void displayBuffer(byte buffer[][3]) {
  for (uint8_t y = 0; y < 6; y++) {           // Loop through each row
...
void scrollText(const char* str, int startX, unsigned int scrollSpeed){
...
void processReceivedCommand(const String& command) {
  ...
  if (command.startsWith("@speed ")) {
    ...
    if (newSpeed > 0) {                                   // Ensure the speed is valid
      ...
      Serial.print(F("Scroll speed updated to: "));
      ...
      Serial.println(F("Invalid speed value."));
    }
  }
  ...
  //dice
    ...
    Serial.println(F("Text saved to EEPROM!"));
...
void help() {
  ...
  Serial.println(F("@speed <value>  - Set scrolling speed in ms"));
  Serial.println(F("@help           - this help message."));
  Serial.println(F("<text>          - Display and scroll the text."));
  Serial.println(F("@ani1           - animation 1: odd-even"));
  Serial.println(F("@ani2           - animation 2: pixel-scan"));
  Serial.println(F("@ani3           - animation 3: sinewave"));
  Serial.println(F("@ani4           - animation 4: DVD screensaver"));
  Serial.println(F("@ani5           - animation 5: tetris"));
  Serial.println(F("@bar            - animation 6: simulated bar VU-Meter"));
  Serial.println(F("@spec           - animation 7: simulated 24-channel spectrum-analysis"));
  Serial.println(F("@timer          - starts a timer of 10min"));
  Serial.println(F("@dice           - random number from 1 to 6"));
}
...
void ani1() {
  Serial.println(F("Displaying animation 1..."));
  //frame 1
  static byte f1[6][3] = {{B10101010,B10101010,B10101010},
                   ...
                   {B01010101,B01010101,B01010101}};
  //frame 2
  static byte f2[6][3] = {{B01010101,B01010101,B01010101},
                   {B10101010,B10101010,B10101010}, 
                   {B01010101,B01010101,B01010101},
                   {B10101010,B10101010,B10101010},
                   {B01010101,B01010101,B01010101},
  ...
  static const byte* frames[2]={(const byte*)f1,(const byte*)f2};
...
void displayAnimation(const byte* frames[], int numFrames, unsigned int animationSpeed) {
...
void ani2() {
  ...
  Serial.println(F("Displaying animation 2..."));
  int xpos = 0;
  int ypos = 0;
  unsigned long lastFrameTime = 0;                  // Timestamp of the last frame change
...
void ani3() {
  ...
  Serial.println(F("Displaying animation 3..."));
...
void ani4() {
  ...
  Serial.println(F("Displaying animation 4..."));
...
void ani5() {
  Serial.println(F("Displaying Tetris Animation..."));
...
void ani6(){
  Serial.println(F("Displaying Simulated Bar Animation..."));
  ...
  while (Serial.available() == false) {                 // Run animation until input is detected
    ...
    if (currentTime - lastFrameTime >= scrollSpeed) {
      ...
      channel1 = random(0, 256);                        // Generate random values for both channels (simulating real input)
...
//display a 24-channel spectrum
void displaySpectrum(const byte* spectrum) {

  displayClear();

  for (uint8_t x = 0; x < 24; x++) {                // Iterate over all 24 columns
    uint8_t value = spectrum[x];                    // Get the spectrum value (0-255)
    uint8_t barHeight = map(value, 0, 255, 0, 6);   // Scale value to 0-6 pixels

    for (uint8_t y = 0; y < barHeight; y++) {       // Draw the vertical bar
...
//test function for displaying a 24-channel spectrumanalysis
void aniSpectrum() {

  Serial.println(F("Displaying Spectrum Animation..."));
  ...
  while (Serial.available() == false) {                   // Run until input is detected
    ...
    if (currentTime - lastFrameTime >= scrollSpeed) {     // Update only at the scroll speed
      ...
      for (uint8_t i = 0; i < 24; i++) {                  // Generate new random values for the spectrum
...
void starttimer() {
  ...
  Serial.println(F("Displaying Timer Animation..."));
  ...
  while (Serial.available() == false) {                                       // Run until input is detected
    ...
    if (currentTime - lastUpdateTime >= 1000) {                               // Update every 1000ms
      ...
      snprintf(timeString, sizeof(timeString), "%d:%02d", minutes, seconds);  // Format the time as "MM:SS"
      ...
      if (seconds == 60) {
        ...
        if (minutes == 10) {                                                  // Prevent overflow beyond 9:99
          minutes = 0;
          Serial.println(F("TIMER DONE! 10MIN OVER!"));
        }
      }
    }
    displayClear();
